...
from fetch_data import fetch_fred_series, fetch_stooq_daily, latest_on_or_before, prior_month_value
...
def update_for_month(target_date: str) -> None:
    ...
    for _, metric in config.iterrows():
        if (target_date, metric["MetricID"]) not in existing:
            new_rows.append({
                "Date": target_date,
    ...
    for _, metric in config.iterrows():
        ...
        mask = (obs["Date"].astype(str) == target_date) & (obs["MetricID"].astype(str) == metric_id)
        ...
        if source_type == "FRED":
            ...
            try:
                ...
                obs.loc[mask, "Notes"] = f"Auto-update failed for derived DGS30-DFII30: {e}"
                continue
        ...
        score = score_from_tilt(classification["tilt"], int(classification["strength"]), metric)
        ...
        obs.loc[mask, "MonthlyChange"] = "" if monthly_change == "" else round(float(monthly_change), 4)
        ...
        obs.loc[mask, "WeightUsed"] = max(float(metric["Weight_Citrini"]), float(metric["Weight_Green"]), float(metric["Weight_Alden"]))
        obs.loc[mask, "Notes"] = f"Auto observation date {obs_date}. {classification['notes']}"
    ...
    # Scenario summary.
    grouped = obs.groupby("Date", as_index=False)[["CitriniScore", "GreenScore", "AldenScore", "OtherScore"]].sum()
    rows = []
    for _, r in grouped.iterrows():
        ...
        rows.append({
            ...
            "Alden": values["Alden"],
            "Other": values["Other"],
            "Leader": leader if sorted_values[0] > 0 else "Insufficient data",
            "Confidence": confidence if sorted_values[0] > 0 else "Low",
        })
...
if __name__ == "__main__":
    ...
    parser.add_argument("--date", default=date.today().strftime("%Y-%m-%d"), help="Target month-end date, YYYY-MM-DD")
